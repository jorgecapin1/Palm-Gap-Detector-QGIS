...
def _build_work_units(all_points, point_index, points_layer, lots_layer, lot_field):
    ...
    if QgsWkbTypes.geometryType(lots_layer.wkbType()) != QgsWkbTypes.PolygonGeometry:
...
def _crs_uses_meter_units(crs):
    ...
    unit = crs.mapUnits()
    if hasattr(QgsUnitTypes, "DistanceMeters") and unit == QgsUnitTypes.DistanceMeters:
        return True
    if hasattr(Qgis, "DistanceUnit"):
        for candidate in ("Meters", "Meter", "DistanceMeters"):
            if hasattr(Qgis.DistanceUnit, candidate) and unit == getattr(Qgis.DistanceUnit, candidate):
                return True
    return False
...
def _writer_no_error():
    ...
    if hasattr(QgsVectorFileWriter, "WriterError"):
