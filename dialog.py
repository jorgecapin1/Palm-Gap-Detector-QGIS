...
def _qt_align_right():
    ...
    if alignment_flag is not None:
        return alignment_flag.AlignRight
    return getattr(Qt, "AlignRight")


def _qt_align_vcenter():
    alignment_flag = getattr(Qt, "AlignmentFlag", None)
    if alignment_flag is not None:
        return alignment_flag.AlignVCenter
    return getattr(Qt, "AlignVCenter")
...
class PalmGapDetectorDialog(QDialog):
    ...
    def _load_layers(self):
        ...
        for layer in QgsProject.instance().mapLayers().values():
            ...
            if geom_type == _geometry_type_value("PointGeometry"):
                self.point_layers.append(layer)
                self.points_combo.addItem(layer.name(), layer.id())
            elif geom_type == _geometry_type_value("PolygonGeometry"):
